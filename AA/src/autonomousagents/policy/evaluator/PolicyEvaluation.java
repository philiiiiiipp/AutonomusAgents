...
public class PolicyEvaluation
{
	...
	public static ValueMap evaluate(final Policy predatorPolicy, final Policy preyPolicy)
	{
		...
		do
		{
			...
			for (State s : predatorPolicy.getPolicy().keySet())
			{
				...
				valueMap.setValueForState(s, maximisation(s, valueMap, predatorPolicy, preyPolicy));

				delta = Math.max(delta, Math.abs(v - valueMap.getValueForState(s)));
			}
			i = i + 1;
	 ...
	 * @return
	 */
	private static float maximisation(final State s, final ValueMap valueMap, final Policy predatorPolicy,
